...
import { useBilling, TClient, TQuoteItem } from '../hooks/useBilling';
...
const CreateQuoteModal: React.FC<Props> = ({ visible, onClose }) => {
  const { clients, validateSiret, saveClient, loading, createQuote, addQuoteItems } = useBilling() as any;

  const [step, setStep] = useState<Step>('client');
  const [selectedClientId, setSelectedClientId] = useState<string>('');
  const [isNewClient, setIsNewClient] = useState(false);
  const [clientSiret, setClientSiret] = useState('');
  const [newClient, setNewClient] = useState<Partial<TClient>>({ is_company: true, address: '', postal_code: '', city: '', country: 'France' });
  ...
  const [items, setItems] = useState<TQuoteItem[]>([{ description: '', quantity: 1, unit_price: 0, total_ht: 0, vat_rate: 20 }]);

  const totals = useMemo(() => {
    const subtotal_ht = items.reduce((s, i) => s + (i.total_ht || 0), 0);
    const vat_amount = subtotal_ht * (vatRate / 100);
  ...
  const reset = () => {
    ...
    setValidityDate('');
    setVatRate(20);
    setPaymentTerms('Paiement à réception');
  ...
  const handleValidateSiret = async () => {
    ...
    if (data) {
      setNewClient((prev) => ({ ...prev, company_name: data.company_name, siret: data.siret, address: data.address, postal_code: data.postal_code, city: data.city, country: data.country, is_company: true }));
    }
  };
  ...
  const updateItem = (idx: number, patch: Partial<TQuoteItem>) => {
    setItems((arr) => {
      const next = [...arr];
      next[idx] = { ...(next[idx] as any), ...(patch as any) } as TQuoteItem;
      next[idx].total_ht = (next[idx].quantity || 0) * (next[idx].unit_price || 0);
      return next;
    });
  };
  ...
  const handleCreate = async () => {
    ...
    if (isNewClient) {
      ...
      if (!ok) return;
  const match = clients.find((c: TClient) => (newClient.siret && c.siret === newClient.siret) || (newClient.company_name && c.company_name === newClient.company_name) || (newClient.email && c.email === newClient.email));
    ...
    if (q && (q as any).id) {
      await addQuoteItems((q as any).id, items);
      onClose();
      reset();
    }
  };
  ...
  return (
    <Modal visible={visible} animationType="slide" onRequestClose={onClose}>
      <ScrollView style={styles.container} contentContainerStyle={{ padding: 16 }}>
        <Text style={styles.title}>Créer un devis légal</Text>
        ...
        {step === 'client' && (
          <View style={{ gap: 12 }}>
            ...
            <View style={{ flexDirection: 'row', justifyContent: 'flex-end', gap: 8 }}>
              ...
              <TouchableOpacity disabled={!canProceedClient} onPress={() => setStep('details')} style={[styles.btn, { backgroundColor: canProceedClient ? '#0ea5e9' : '#bae6fd' }]}>
        ...
        {step === 'details' && (
          <View style={{ gap: 12 }}>
            <View style={{ flexDirection: 'row', gap: 8 }}>
              <View style={{ flex: 1 }}>
                <Text style={styles.label}>Validité (YYYY-MM-DD)</Text>
                <TextInput placeholder="YYYY-MM-DD" value={validityDate} onChangeText={setValidityDate} style={styles.input} />
              </View>
              <View style={{ flex: 1 }}>
            ...
            <ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={{ gap: 8 }}>
              {['Paiement à réception', 'Paiement à 15 jours', 'Paiement à 30 jours'].map((opt) => (
            ...
            <ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={{ gap: 8 }}>
              {['Virement bancaire', 'Chèque', 'Espèces', 'Carte bancaire'].map((opt) => (
            ...
            <View style={{ flexDirection: 'row', justifyContent: 'space-between', gap: 8 }}>
              ...
              <TouchableOpacity onPress={() => setStep('items')} style={[styles.btn, { backgroundColor: '#0ea5e9', flex: 1 }]}>
                <Text style={styles.btnText}>Suivant</Text>
              </TouchableOpacity>
            </View>
          </View>
          ...
          <View style={{ gap: 12 }}>
            {items.map((item, idx) => (
              <View key={idx} style={styles.card}>
                ...
                <Text style={styles.muted}>Total HT: {((item.total_ht || 0)).toFixed(2)} €</Text>
                {items.length > 1 && (
            ...
            <View style={styles.card}>
              ...
              <View style={{ flexDirection: 'row', justifyContent: 'space-between' }}>
                ...
                <Text style={styles.value}>{totals.subtotal_ht.toFixed(2)} €</Text>
              </View>
              <View style={{ flexDirection: 'row', justifyContent: 'space-between' }}>
                <Text style={styles.muted}>TVA ({vatRate}%)</Text>
                <Text style={styles.value}>{totals.vat_amount.toFixed(2)} €</Text>
              </View>
              <View style={{ flexDirection: 'row', justifyContent: 'space-between', marginTop: 6 }}>
                <Text style={[styles.value, { fontSize: 18 }]}>Total TTC</Text>
                <Text style={[styles.value, { fontSize: 18 }]}>{totals.total_ttc.toFixed(2)} €</Text>
              </View>
            </View>
            ...
            <View style={{ flexDirection: 'row', justifyContent: 'space-between', gap: 8 }}>
              ...
              <TouchableOpacity onPress={handleCreate} disabled={loading || !canProceedItems || !validityDate} style={[styles.btn, { backgroundColor: loading || !canProceedItems || !validityDate ? '#bae6fd' : '#0ea5e9', flex: 1 }]}>
                {loading ? <ActivityIndicator color="#fff" /> : <Text style={styles.btnText}>Créer le devis</Text>}
              </TouchableOpacity>
            </View>
          </View>
        )}
