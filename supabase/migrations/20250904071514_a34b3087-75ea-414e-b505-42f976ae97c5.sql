...
CREATE OR REPLACE FUNCTION public.update_updated_at_mission_tables()
RETURNS TRIGGER AS $$
BEGIN
  NEW.updated_at = now();
  RETURN NEW;
...
CREATE TRIGGER update_inspection_departures_updated_at
  BEFORE UPDATE ON public.inspection_departures
  FOR EACH ROW
  EXECUTE FUNCTION public.update_updated_at_mission_tables();

CREATE TRIGGER update_inspection_arrivals_updated_at
  BEFORE UPDATE ON public.inspection_arrivals
  FOR EACH ROW
  EXECUTE FUNCTION public.update_updated_at_mission_tables();

CREATE TRIGGER update_mission_costs_updated_at
  BEFORE UPDATE ON public.mission_costs
  FOR EACH ROW
  EXECUTE FUNCTION public.update_updated_at_mission_tables();

CREATE TRIGGER update_mission_documents_updated_at
