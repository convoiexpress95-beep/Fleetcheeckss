...
export default function FileUploadButton({ onFileUploaded, disabled }: FileUploadButtonProps) {
  ...
  const handleFileSelect = (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (file) {
      ...
      if (!allowedTypes.includes(file.type)) {
        toast({ title: "Type non autorisé", description: "Images, PDF, Word, texte", variant: "destructive" });
        return;
      }
  ...
  const handleUpload = async () => {
    ...
    try {
      ...
      toast({ title: "Fichier téléchargé", description: "Pièce jointe prête" });
    } catch (error) {
    ...
    <div className="flex items-center gap-2">
      ...
      {selectedFile ? (
        <div className="flex items-center gap-2 bg-muted rounded-lg px-3 py-2">
          ...
          <Button size="sm" variant="ghost" onClick={clearSelection} className="h-6 w-6 p-0">✕</Button>
          <Button size="sm" onClick={handleUpload} disabled={uploading} className="h-6 px-2">
        ...
        <Button size="sm" variant="ghost" onClick={() => fileInputRef.current?.click()} disabled={disabled} className="h-8 px-2">
