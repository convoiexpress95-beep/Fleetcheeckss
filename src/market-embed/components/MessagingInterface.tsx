...
export default function MessagingInterface({ userId }: { userId: string }) {
  const RTC_DEBUG = (import.meta as any).env?.VITE_RTC_DEBUG === '1';
  ...
  useEffect(() => {
    const ch = supabase
      ...
      .on('postgres_changes', { event: 'INSERT', schema: 'public', table: 'messages', filter: `sender_id=neq.${userId}` }, async (payload) => {
        RTC_DEBUG && console.debug('[RTC] INSERT messages payload:', payload);
    ...
    const subscription = ch.subscribe((status) => {
      if (RTC_DEBUG) console.debug('[RTC] channel status:', status);
    });
    if (RTC_DEBUG) console.debug('[RTC] subscribing to channel: messages');
    return () => {
      if (RTC_DEBUG) console.debug('[RTC] removing channel: messages');
  ...
  async function fetchMessages(conversationId: string) {
    try {
      ...
      // 2) Construire la map profils par sender_id via profiles.user_id
      ...
      const enriched = list.map((m) => ({ ...m, profiles: profilesMap[m.sender_id] || null }));
